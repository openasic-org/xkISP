#include "../src/dehaze.h"

int main(int argc, char** argv)
{
    top_register topParam;
    dehaze_register dehaze_param;
    stream_u42 src;
    stream_u42 dst;
    ...
    printf("\tTest for ISP dehaze module!\n");
    memset(&topParam, 0, sizeof(top_register));
    memset(&dehaze_param, 0, sizeof(dehaze_register));

    topParam.frameWidth = 640;
    topParam.frameHeight = 480;
    dehaze_param.m_nEb=1;
    ...
    //In
    FILE* fp_r1 = fopen(DEHAZE_SRC1, "rb");
    if (!fp_r1) {
    ...
    //Golden
    FILE *fp_g1 = fopen(DEHAZE_GOLDEN1, "r");
    if(!fp_g1){
    ...
    //Execution
    dehaze(topParam,dehaze_param,src,dst);
    printf("\tExecution completed!\n");
    printf("\t%d\n",dst.size());
    //Out
    FILE *fp_w1 = fopen(DEHAZE_DST1, "w");
    if(!fp_w1){
