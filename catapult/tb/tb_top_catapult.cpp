...
while((ptr_src=readdir(dir_raw))!=NULL && (ptr_golden=readdir(dir_yuv))!=NULL && (ptr_cfg=readdir(dir_cfg))!=NULL)
{
    if(ptr_cfg->d_name[0] == '.')
        continue;
    cfg_name = ptr_cfg->d_name;
    std::string path_file_cfg = path_cfg + cfg_name;
    top_cfg = path_file_cfg.c_str();
    printf("cfg file = %s\n", top_cfg);

    char buf[100] = "";
    FILE* fp_cfg1 = fopen(top_cfg, "r");
    char *p, *q;
